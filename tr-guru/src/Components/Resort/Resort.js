...
import TuristPointData from '../FakeData/TouristPointData';
...
const Resort = () => {
    const [resort, setResort] = useState({
        id:TouristPointData[0].id,
        title:TouristPointData[0].title,
        description:TouristPointData[0].description,
        image:TouristPointData[0].image
    })

const handleResort = id =>{
    const newPoint= TouristPointData.find(data => id ===data.id)
    setResort(newPoint)
}
    return (
